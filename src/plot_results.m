...
xyfontSize=16;
    include = [1,2,5,8];
    include = 1:i_CE-1;
for i_column = 1:length(include)
    coloring_vec = get_coloring_vec(CE_stats,include(i_column),s_da_0_cell,params,rho,false);
    projected_psi = CE_stats{1,include(i_column)}'*pc(:,1:2);
    ax1=subplot(2,8,i_column);
    
    scatter(projected_psi(find(1-coloring_vec),1),projected_psi(find(1-coloring_vec),2),[],'+','MarkerEdgeColor','k');
    hold on;
    scatter(projected_psi(find(coloring_vec),1),projected_psi(find(coloring_vec),2),[],'o','MarkerEdgeColor','c');
    hold off;
    colormap(ax1,summer);
    if(i_column>1)
%         set(ax1,'xtick',[],'ytick',[],'fontsize',xyfontSize);
    ...
    title(['Iteration ',num2str(include(i_column))],'FontSize',16);
