function [theta_psi,phi_hist_united,finished_idx,v_pi_local] =  ...
    extractVIData_case24(iterationDir,N_psi,PSI_DIRNAME_PREFIX,job_output_filename,params)
history_size = params.N_s_da_0*params.N_episode*params.num_of_days*24;
num_united_histories = round(3e4/history_size);
sparsity_level = ceil(N_psi/num_united_histories);
finished_idx = [];
...
for i_psi=1:N_psi
    try
        ...
        if(numOfExistingMatFiles>0) %if job finished and there's an output
            ...
            for i_matFile=1:length(outputFileList)
                ...
                if(strcmp(outputFileName,job_output_filename))
                    ...
                    v_pi_local(i_psi) = mean(loaded_file.theta'*loaded_file.phi_hist);
        ...
        warning(['Problem using extractVIData_case24 for iteration = ' num2str(i_psi)]);
